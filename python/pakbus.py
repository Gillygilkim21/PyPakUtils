...
def PakBus_hdr(DstNodeId, SrcNodeId, HiProtoCode, ExpMoreCode = 0x2, LinkState = 0xA, Priority = 0x1, HopCnt = 0x0, DstPhyAddr = None, SrcPhyAddr = None):
    ...
    print " ".join(["%02x"%(ord(x)) for x in hdr])
    return hdr
...
#
def pkt_hello_cmd(DstNodeId, SrcNodeId, IsRouter = 0x00, HopMetric = 0x02, VerifyIntv = 1800, HopCnt = 0x0):
    ...
    hdr = PakBus_hdr(DstNodeId, SrcNodeId, 0x0, 0x1, 0x9, Priority = 3, HopCnt = HopCnt) # PakBus Control Packet
...
# Create Hello Command packet
#
def pkt_hello_cmd_multihop(DstNodeId, SrcNodeId, DstPhyAddr, IsRouter = 0x00, HopMetric = 0x02, VerifyIntv = 1800, HopCnt = 0x0):
    # DstNodeId:   Destination node ID (12-bit int)
    # SrcNodeId:   Source node ID (12-bit int)
    # IsRouter:    Flag if source node is a router (default: 0)
    # HopMetric:   Worst case interval to complete transaction (default: 0x02 -> 5 s)
    ...
    print "multihop packet dst = ", DstNodeId, "dstPhyAddr = ", DstPhyAddr
    hdr = PakBus_hdr(DstNodeId, SrcNodeId, 0x0, 0x1, 0x9, Priority = 3, DstPhyAddr = DstPhyAddr, HopCnt = HopCnt) # PakBus Control Packet
    msg = encode_bin(['Byte', 'Byte', 'Byte', 'Byte', 'UInt2'], [0x09, TranNbr, IsRouter, HopMetric, VerifyIntv])
    pkt = hdr + msg
    return pkt, TranNbr
...
#
def ping_node(s, DstNodeId, SrcNodeId, IsRouter = 0x0, HopCnt = 0x0):
    # s:            Socket object
    # DstNodeId:    Destination node ID (12-bit int)
    # SrcNodeId:    Source node ID (12-bit int)

    # send hello command and wait for response packet
    print 'DstNodeId: ', DstNodeId
    print 'SrcNodeId: ', SrcNodeId
    print 'IsRouter: ', IsRouter
    print 'HopCnt: ', HopCnt
    if DstNodeId > 1:
        pkt, TranNbr = pkt_hello_cmd_multihop(DstNodeId, SrcNodeId, DstPhyAddr=1, IsRouter = IsRouter, HopCnt = HopCnt)
    else:
        pkt, TranNbr = pkt_hello_cmd(DstNodeId, SrcNodeId, IsRouter = IsRouter, HopCnt = HopCnt)
    send(s, pkt)
    hdr, msg = wait_pkt(s, DstNodeId, SrcNodeId, TranNbr, timeout = 6)
